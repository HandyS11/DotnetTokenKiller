...
down to only what matters. Prefix `build`, `test`, `restore`, `clean`, and `format` with `dtk` for
60–90% fewer tokens — per-command filters, token analytics, and one-command setup for 8 AI coding
agents.
