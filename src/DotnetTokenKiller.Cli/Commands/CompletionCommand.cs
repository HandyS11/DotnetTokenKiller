    ...
    : AsyncCommand<CompletionCommandSettings>
{
    private const string BashCompletion =
        ...
        _dtk_completion() {
            ...
            local init_providers="claude copilot copilot-cli gemini codex opencode cursor windsurf aider jetbrains"
    ...
    private const string PowerShellCompletion =
        ...
        Register-ArgumentCompleter -Native -CommandName dtk -ScriptBlock {
            ...
            $providers = @('claude', 'copilot', 'copilot-cli', 'gemini', 'codex', 'opencode', 'cursor', 'windsurf', 'aider', 'jetbrains')
            $configCmds = @('show', 'set')
            $shells = @('bash', 'zsh', 'fish', 'powershell')
