...
/// <summary>Runs dotnet publish with filtered output.</summary>
/// <param name="filteredRun">The filtered run use case.</param>
/// <param name="filter">The publish output filter.</param>
internal sealed class DotnetPublishCommand(
    FilteredRunUseCase filteredRun,
    [FromKeyedServices(FilterKeys.Publish)] IOutputFilter filter) : AsyncCommand<DotnetCommandSettings>
{
    /// <inheritdoc/>
