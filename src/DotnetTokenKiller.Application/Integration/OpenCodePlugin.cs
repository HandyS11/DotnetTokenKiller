...
/// plugin uses <c>spawn</c> without a shell; and OpenCode v1 silently drops a plugin with any non-function export.
/// </para>
/// <para>
/// The plugin resolves <c>dtk</c> from <c>PATH</c> itself and spawns the absolute path. Given a bare name on Windows,
/// libuv (Node) and Bun look in the current directory before <c>PATH</c>; OpenCode's current directory is the project,
/// and the plugin runs before OpenCode's permission check, so a <c>dtk.exe</c> committed to a repository would
/// otherwise run unapproved. Empty and relative <c>PATH</c> entries are skipped for the same reason, and off Windows a
/// candidate must be executable, as a shell requires. No <c>dtk</c> on <c>PATH</c> means no process and no rewrite.
/// </para>
/// </remarks>
internal static class OpenCodePlugin
{
    /// <summary>The call a plugin still runs dtk through, even after local edits.</summary>
    internal const string InvocationSignature = "[\"hook\", \"opencode\"]";
    ...
    internal const string Body =
        ...
        // first, so a dtk.exe planted in a repository would run before OpenCode asks for permission.
        function findDtk() {
