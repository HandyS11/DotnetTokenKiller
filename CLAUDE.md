...
- `cold-start` times the built `dtk` binary end to end in three scenarios, 55 dtk spawns each (the
  ...
  ext4 is not in these figures. Needs a POSIX shell. Measured 2026-09-13, before → after starting
  tracking setup in the background and turning `TieredPGO` off: pipe 295.9 →
  230.2 ms; wrapped overhead 286.6 → 228.2 ms
  (instant child) and 288.4 → 185.5 ms (1000 ms child,
  wall-clock 1188.0 ms).
