...
public class CountingTextWriterTests
{
    ...
    [Fact]
    public async Task EncodingNewLineFlushAndWriteAsyncChar_AreTheInnerWritersAndNotCounted()
    {
        var seen = new List<string>();
        var counter = new ChunkedTokenCounter(TokenizerModel.Cl100kBase, int.MaxValue, (text, _) =>
        {
        ...
        sut.NewLine = "\r\n";
        await sut.WriteAsync('x');
        await sut.FlushAsync();
        counter.Finish(string.Empty);
        await counter.TotalAsync();

        sut.Encoding.Should().BeSameAs(inner.Encoding);
        inner.NewLine.Should().Be("\r\n");
        inner.ToString().Should().Be("x");
